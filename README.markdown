...
* `validate_string`: Validates that all passed values are string data structures. Aborts catalog compilation if any value fails this check.
  ...
  Undef will also pass true as it is interpreted as an empty string when passed to a function
  ```
  ...
  When future parser is enabled numbers not quoted will also fail compilation:
  ```
  validate_string(1)
  validate_string(1.0)
  ```
